...
Gestión de perfiles de importación (`ImportProfile`): alta, consulta, edición y baja vía API REST. Cada perfil define, por banco, cómo interpretar el formato de un extracto (CSV o PDF con tabla) — es la base sobre la que se construirán los parsers de documentos (`DocumentParserInterface`).
