...
Interpretación de ficheros CSV según un `ImportProfile` (`source_format = csv`) para obtener la lista de movimientos crudos (`ParsedMovement`) que contienen, como primer adaptador de `DocumentParserInterface`. Es la base sobre la que se construirá el pipeline de importación de documentos.
